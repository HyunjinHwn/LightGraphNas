...
def list_rank_loss(loss_syn_list, loss_full_list, rank_norm='minmax', 
                   rank_dis='L2', rank_sum='mean', gamma=1):
    """
    Compute ListNet loss between two lists using softmax and cross-entropy.
    
    Args:
        loss_syn_list (torch.Tensor): Predicted relevance scores (1D Tensor).
        loss_full_list (torch.Tensor): Ground-truth relevance scores (1D Tensor).
        
    Returns:
        torch.Tensor: Computed ListNet loss.
    """
    # Ensure the inputs are tensors
    loss_syn_list = torch.tensor(loss_syn_list, dtype=torch.float32) if not isinstance(loss_syn_list, torch.Tensor) else loss_syn_list
    loss_full_list = torch.tensor(loss_full_list, dtype=torch.float32) if not isinstance(loss_full_list, torch.Tensor) else loss_full_list
    
    # Apply softmax to both lists
    if rank_norm == 'softmax':
        prob_syn = F.softmax(loss_syn_list, dim=0)
        prob_full = F.softmax(loss_full_list, dim=0)
    elif rank_norm == 'minmax':
        prob_syn = (loss_syn_list - torch.min(loss_syn_list)) / (torch.max(loss_syn_list) - torch.min(loss_syn_list))
        prob_full = (loss_full_list - torch.min(loss_full_list)) / (torch.max(loss_full_list) - torch.min(loss_full_list))
        torch.clamp(prob_syn, 1e-8, 1-1e-8)
        torch.clamp(prob_full, 1e-8, 1-1e-8)
    elif rank_norm == 'none':
        prob_syn = loss_syn_list
        prob_full = loss_full_list
    # Compute cross-entropy loss manually
    if rank_dis == 'CE':
        assert(rank_norm != 'none')
        lower_bound = -prob_full * torch.log(prob_full + 1e-8)
        loss = -prob_full * torch.log(prob_syn + 1e-8) - lower_bound
    elif rank_dis == 'L1':
        loss = torch.abs(prob_syn - prob_full)
    elif rank_dis == 'L2':
        loss = (prob_syn - prob_full)**2
    if rank_sum == 'mean':
        loss = torch.mean(loss)
    elif rank_sum == 'weight':
        # normalize prob_full to 0 ~ 1
        normed = (prob_full - prob_full.min()) / (prob_full.max() - prob_full.min() + 1e-8)
        # reverse weight so that low prob gets higher weight
        weight = (1.0 - normed).pow(gamma)
        loss = weight * loss
        loss = torch.sum(loss)
    return loss
