...
def row_normalize_tensor(mx):
    eps = 1e-8
    return mx / (mx.norm(p=2, dim=1, keepdim=True) + eps)
...
def adj_to_edge_index_weight(adj, device=None):
    """
    Convert adjacency matrix (dense/sparse/numpy/scipy) 
    into PyTorch Geometric (edge_index, edge_weight).
    
    Parameters
    ----------
    adj : torch.Tensor | torch.sparse.Tensor | scipy.sparse matrix | np.ndarray
        The adjacency matrix.
    device : torch.device or str, optional
        Device to move tensors to. If None, inferred from adj (if tensor), else 'cpu'.
    ...
    if isinstance(adj, torch.Tensor) and adj.is_sparse:
        adj = adj.coalesce()
        edge_index = adj.indices().to(device)
        edge_weight = adj.values().to(device)
        return edge_index, edge_weight

    # --- Case 2: torch_sparse.SparseTensor ---
    elif isinstance(adj, SparseTensor):
        # row/col/value 추출
        row, col, val = adj.storage.row(), adj.storage.col(), adj.storage.value()
        edge_index = torch.stack([row, col], dim=0).to(device)
        edge_weight = val.to(device) if val is not None else None
        return edge_index, edge_weight
    ...
    # --- Case 3: scipy sparse matrix ---
    elif sp.issparse(adj):
        adj = adj.tocoo()
        row = torch.from_numpy(adj.row).long().to(device)
        col = torch.from_numpy(adj.col).long().to(device)
        edge_index = torch.stack([row, col], dim=0)
        edge_weight = torch.from_numpy(adj.data).float().to(device)
        return edge_index, edge_weight
