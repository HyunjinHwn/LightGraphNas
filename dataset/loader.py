...
class TransAndInd:
    ...
    def retrieve_class_sampler(self, edge_index, args, batch_size=256, shuffle=True):
        ...
        for i in range(self.nclass):
            ...
            samplers.append(build_edge_index_batches(edge_index, node_idx, sizes, 
                                                              batch_size=batch_size, shuffle=shuffle,
                                                              num_workers=0, device=args.device))
        return samplers
                # usage
                # for n_id, target, edge_index_batches in self.samplers[i]:
                #     out = model(x[n_id], edge_index_batches, sampler=True)
...
def build_edge_index_batches(edge_index, input_nodes, sizes, batch_size=256, 
                            num_workers=0, device=None, shuffle=True):
    """
    returns: generator of (n_id, edge_index_batches)
            where edge_index_batches = [(edge_index, e_id, size), ...]
    """
    loader = NeighborSampler(
        edge_index,
        node_idx=input_nodes,    # 배치로 뽑을 타겟 노드 집합 (e.g., 학습 대상)
        sizes=sizes,             # 예: [15, 10, 5] (얕은 -> 깊은 레이어 순)
        batch_size=batch_size,
        shuffle=shuffle,
        num_workers=num_workers,
        return_e_id=True         # e_id를 받아야 edge_weight 슬라이싱 가능
    )
    for batch_size, n_id, edge_indices in loader:
        # edge_indices: 얕은 레이어 -> 깊은 레이어 순서의 리스트
        if len(edge_indices[0]) != 3:
            edge_indices = [edge_indices]
        edge_index_batches = []
        for edge_index, e_id, size in edge_indices:
            if device is not None:
                edge_index = edge_index.to(device)
                e_id = e_id.to(device)
            edge_index_batches.append((edge_index, e_id, size))
        target = n_id[:batch_size]   # 또는 n_id[:edge_index_batches[-1][2][1]]
        yield n_id, target, edge_index_batches
